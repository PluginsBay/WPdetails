...
<?php
    ...
    $entries = $wpdb->get_results( "SELECT  id,platform,knp_date FROM {$wpdb->prefix}wpdwid WHERE knp_ts >= '" . $from_ts . "' AND  knp_ts <= '" . $to_ts . "' ORDER BY id DESC");
 
        $common_platforms = array('"".__("Apple","wpdlang").""','"".__("Windows","wpdlang").""','"".__("Linux","wpdlang").""','"".__("Android","wpdlang").""');
...
<?php if(sizeof($platform_stats) > 0){ ?>
      <div class="chartBox">
        <h4 class='widhead'><?php _e('Platforms used in last', 'wpdlang');  echo $days; _e('days', 'wpdlang'); ?></h4>
...
<script type="text/javascript">
       ...
       var myChart18 = echarts.init(document.getElementById('platform_type')); 
        ...
        var option = {
                ...
                series: [
                    {
                        name: '<?php echo __("Total Visits","wpdlang"); ?>',
                        type: 'line',
                        smooth: true,
                        symbol:'none',
                        symbolSize:2,
...
<?php } else { echo __("No Data Recorded Yet!","wpdlang"); } ?>
...
<?php if(sizeof($series_data) > 0){ ?>
    ...
    <div style='text-align:center;'></div><br>
      <div class="chartBox">
        <h4 class='widhead'><?php _e('Platforms Used By Date', 'wpdlang'); ?></h4>
    ...
    options : [
        {
            ...
            legend: {
                ...
                orient:'vertical',
                padding: 0,
                data:['<?php echo __("Apple","wpdlang"); ?>','<?php echo __("Windows","wpdlang"); ?>','<?php echo __("Linux","wpdlang"); ?>','<?php echo __("Android","wpdlang"); ?>','<?php echo __("Others","wpdlang"); ?>']
            },
            toolbox: {
                show : true,
                color : ['#bdbdbd','#bdbdbd','#bdbdbd','#bdbdbd'],
